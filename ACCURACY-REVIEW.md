# Figure & Claim Accuracy Review — Mycroft

*Companion to the layout linter. Layout = geometry (run `npm run audit:layout` first). This = substance.*

## Ground truth for this book

**Expert persona to adopt:** a senior accountant / auditor (CPA-level) and finance-data engineer.

**Domain:** corporate accounting, financial close, reconciliation, controls, and audit evidence, operated through agentic recipes.

**Authoritative sources to check claims against:**
- US GAAP — FASB Accounting Standards Codification (ASC).
- PCAOB auditing standards (e.g. AS 1105, *Audit Evidence* — sufficient + appropriate), and AICPA standards where cited.
- SEC reporting requirements; COSO internal-control framework.
- Accounting identities and mechanics that must hold exactly: debits = credits; subledger sum = GL control-account balance; cash-flow ties; aging buckets that partition without overlap; reconciliations that net to zero.

**Recompute, don't trust the picture:** control totals, tie-outs, aging math, variance %s, and any stated balance. A reconciliation figure that doesn't balance, or an identity drawn unequal, is `[CRITICAL]`.
